...
- **Interactive islands:** embed Preact TSX components in templ views with `@assets.Island("name", props)`; the component file is `app/assets/js/islands/<name>.tsx` (default-export; file path = island name, case-sensitive). `base.templ` loads the bundle via `assets.Scripts()`. After changing frontend sources run `go run . js:build` and commit the dist output (see PLAN.md Phase 3).
