...
cmd/             CLI commands: scaffolding generators, `new` (project
                 ...
                 dump/show, engine list/install, plugin install (deprecated),
                 upload, REPL, version.
  clitemplate/   Embedded project template used by `airway new`. Template files
                 end in .tmpl, with `{{module}}` as the module-path placeholder;
                 keep it in sync when changing skeleton files under app/ etc.
db/
