...
lib/
  app/           App struct: Gin engine construction, CORS, URL_PREFIX handling,
                 routes wiring, and the public http.Handler shared by the web
                 server and desktop wrappers.
  boot/          Programmatic full-stack bootstrap (env pin, migrations, DB,
                 Redis, storage, plugins) from explicit options; desktop
                 wrappers start here instead of reading the environment.
  ...
  migrate/       Migration internals (dialect compiler, schema state) plus the
                 exported engine behind db:migrate (lib/migrate.Run/Rollback/
                 Status over any fs.FS of migration files).
...
- **Interactive islands:** embed Preact TSX components in templ views with `@assets.Island("name", props)`; the component file is `app/assets/js/islands/<name>.tsx` (default-export; file path = island name, case-sensitive). `base.templ` loads the bundle via `assets.Scripts()` and styles via `assets.Stylesheet()`. After changing frontend sources run `go run . js:build` and commit the dist output (see docs/frontend.md).
...
- The default CORS middleware in `lib/app` allows **all origins** with credentials — this is a development default; tighten it before exposing a deployment publicly (desktop wrappers use `boot.Options{StrictCORS: true, SameOriginWS: true}` for same-origin-only).
