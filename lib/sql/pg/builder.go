// Package pg provides a PG-specific SQL builder.
//
// All PostgreSQL features are available: RETURNING, DISTINCT ON, LATERAL JOIN,
// ON CONFLICT, UPDATE...FROM, DELETE...USING, ILIKE, FOR UPDATE/SHARE, JSONB/ARRAY helpers.
package pg
...
// Builder is the pg-specific SQL builder.
type Builder struct {
	inner *sql.Builder
}
...
var (
	...
	JSONGet          = sql.JSONGet
	JSONGetText      = sql.JSONGetText
	JSONPath         = sql.JSONPath
	JSONPathText     = sql.JSONPathText
	JSONContains     = sql.JSONContains
	JSONContainedBy  = sql.JSONContainedBy
	JSONHasKey       = sql.JSONHasKey
	JSONHasAnyKeys   = sql.JSONHasAnyKeys
	JSONHasAllKeys   = sql.JSONHasAllKeys
)
...
// --- PG-only builder methods ---

func (b *Builder) DistinctOn(fields ...string) *Builder {
	b.inner.DistinctOn(fields...)
	return b
}
func (b *Builder) FullJoin(tableName string, on sql.CondBuilder) *Builder {
	b.inner.FullJoin(tableName, on)
	return b
}
func (b *Builder) FullJoinTable(table sql.TableName, on sql.CondBuilder) *Builder {
	b.inner.FullJoinTable(table, on)
	return b
}
func (b *Builder) JoinLateral(query *Builder, alias string, on sql.CondBuilder) *Builder {
	b.inner.JoinLateral(query.inner, alias, on)
	return b
}
func (b *Builder) LeftJoinLateral(query *Builder, alias string, on sql.CondBuilder) *Builder {
	b.inner.LeftJoinLateral(query.inner, alias, on)
	return b
}
...
func (b *Builder) ExceptAll(query *Builder) *Builder {
	b.inner.ExceptAll(query.inner)
	return b
}
func (b *Builder) OnConflictOnConstraintDoNothing(constraint string) *Builder {
	b.inner.OnConflictOnConstraintDoNothing(constraint)
	return b
}
func (b *Builder) OnConflictOnConstraintDoUpdate(constraint string, vals sql.H) *Builder {
	b.inner.OnConflictOnConstraintDoUpdate(constraint, vals)
	return b
}
func (b *Builder) Returning(fields ...string) *Builder {
	b.inner.Returning(fields...)
	return b
}
func (b *Builder) ReturningFields(columns ...sql.FieldName) *Builder {
	b.inner.ReturningFields(columns...)
	return b
}
func (b *Builder) ReturningRefs(columns ...sql.FieldName) *Builder {
